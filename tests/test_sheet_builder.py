    ...
    def test_project_page_link_matches_award_badge_geometry(self) -> None:
        ...
        for stylesheet_name in ("site.css", "style.css"):
            ...
            project_link = re.search(
                r"\.publication-list a\.publication-project-link\s*\{(?P<body>[^}]*)\}",
                stylesheet,
            )
            self.assertIsNotNone(project_link)
            project_link_body = project_link.group("body")
            self.assertIn("font-size: 0.72rem", project_link_body)
            self.assertIn("line-height: 1.35", project_link_body)
            self.assertIn("padding: 3px 8px", project_link_body)
